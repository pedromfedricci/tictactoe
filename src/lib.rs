// no longer required after 1.56 #![feature(bindings_after_at)]
...
impl<const LEN: usize, const COLS: usize> GameBoard<LEN, COLS> {
    ...
    #[inline]
    const fn constraints() -> Result<(), MalformedError> {
        if COLS == 0 {
...
pub fn play_loop<const LEN: usize, const COLS: usize>(mut board: GameBoard<LEN, COLS>) {
    println!("{}", board);
...
fn player_input() -> Result<Pos, IoError> {
    loop {
